...
<div class="mt-header-mobile menu-background-mobile hide-desktop top-nav <?php if(!empty($option['menu_search'])) { if($option['menu_search']=="1") { ?>search-on<?php } } ?>">
	...
	<div class="mt-mobile-logo"><?php boomnews_logo_mobile(); ?></div>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="head container-fluid">
						<div class="head-logo"><?php boomnews_logo(); ?></div>
						<?php if  (!empty($option['menu_top_ad'])) {  ?>
							...
							<?php } else { echo boomnews_top_content(); } ?>
						<?php } else { echo boomnews_top_content(); } ?>
					</div>
				</div>
			</div>
...
function boomnews_top_content() { $option = get_option("boomnews_theme_options"); ?>
		<div class="head-nav">
			...
			?>
			<?php if(!empty($option['url_latest'])) { ?><a class="mt_l_latest <?php if($option['url_latest']==get_the_ID()) { ?>active<?php } ?>" href="<?php echo get_permalink(esc_html($option['url_latest'])); ?>"><?php echo esc_html($t_latest); ?> <span><?php echo esc_html($t_posts); ?></span></a><?php } ?>
			<?php if(!empty($option['url_popular'])) { ?><a class="mt_l_popular <?php if($option['url_popular']==get_the_ID()) { ?>active<?php } ?>" href="<?php echo get_permalink(esc_html($option['url_popular'])); ?>"><?php echo esc_html($t_popular); ?> <span><?php echo esc_html($t_posts); ?></span></a><?php } ?>
			<?php if(!empty($option['url_hot'])) { ?><a class="mt_l_hot <?php if($option['url_hot']==get_the_ID()) { ?>active<?php } ?>" href="<?php echo get_permalink(esc_html($option['url_hot'])); ?>"><?php echo esc_html($t_hot); ?> <span><?php echo esc_html($t_posts); ?></span></a><?php } ?>
						...
						<div class="row">
							<div class="col-md-12">
								...
								<ul class="share">
									...
									<li class="share-twitter"><a class="mt-radius" href="http://twitter.com/home/?status=<?php echo esc_attr($title); ?>" target="_blank"><span><?php echo esc_html__('Tweet on Twitter', 'boomnews'); ?></span></a></li>
									<li class="share-more">
